...
DEFAULTS={"started":False,"finished":False,"score":0,"streak":0,"best":0,"lives":3,"answered":0,"correct":0,"wrong":0,"timed_out":0,"a":2,"b":2,"choices":[],"feedback":"","locked":False,"xp":0,"badges":set(),"game_start":None,"deadline":None,"end_reason":""}
...
def new_question():
    a=random.choice(st.session_state.tables); b=random.randint(0,10); right=a*b; opts={right}
...
def finish(reason):
    st.session_state.finished=True; st.session_state.end_reason=reason; st.session_state.deadline=None
...
def process_answer(value):
    ...
    is_correct=value==st.session_state.a*st.session_state.b
    if is_correct:
        ...
        if not test:
            bonus=min(st.session_state.streak-1,5)*2; st.session_state.score+=10+bonus; st.session_state.xp+=20+bonus
            st.session_state.feedback=f"✅ Juist! +{10+bonus} punten en +{20+bonus} XP"; st.balloons(); update_badges()
    else:
        st.session_state.wrong+=1; st.session_state.streak=0
        if not test:
            st.session_state.lives-=1; st.session_state.feedback=f"💡 Bijna! Het juiste antwoord is {st.session_state.a*st.session_state.b}."
    if test:
...
with st.sidebar:
    ...
    if level=="Met tijdsklok": seconds_per_sum=st.slider("Seconden per rekensom",3,30,10,1,disabled=st.session_state.started)
    else: seconds_per_sum=None
...
if not st.session_state.started:
    st.markdown('<div class="card"><h3>🪐 Kies je uitdaging</h3><p><b>Gewoon:</b> oefenen zonder klok.<br><b>Met tijdsklok:</b> kies zelf de tijd per rekensom. Na het verstrijken gaat het spel automatisch verder.<br><b>Toetsniveau:</b> 100 invulvragen in maximaal 10 minuten, zonder feedback tijdens de toets.</p></div>',unsafe_allow_html=True)
    if not tables: st.warning("Kies minstens één tafel.")
    if st.button("🚀 Start avontuur",use_container_width=True,disabled=not tables):
        st.session_state.started=True; st.session_state.tables=tables; st.session_state.level=level; st.session_state.answer_mode=answer_mode; st.session_state.rounds=rounds; st.session_state.seconds_per_sum=seconds_per_sum; st.session_state.game_start=time.time()
    ...
    def game_panel():
        ...
        if st.session_state.finished:
            ...
            pct=round(100*st.session_state.correct/max(1,total),1)
            elapsed=min(time.time()-st.session_state.game_start,600 if st.session_state.level=="Toetsniveau" else 10**9)
            medal="🏆" if pct>=90 else "🌟" if pct>=70 else "💪"
            st.markdown(f'<div class="card" style="text-align:center"><div style="font-size:4rem">{medal}</div><h2>Resultaat</h2><p>{st.session_state.end_reason}</p><h1>{pct}%</h1></div>',unsafe_allow_html=True)
            c1,c2,c3,c4=st.columns(4)
            c1.metric("✅ Juist",st.session_state.correct); c2.metric("❌ Fout",st.session_state.wrong); c3.metric("⏭️ Geen antwoord",st.session_state.timed_out); c4.metric("⏱️ Tijd",format_time(elapsed))
        ...
        if st.session_state.level=="Toetsniveau":
            c1.metric("Vraag",f"{st.session_state.answered+1}/100"); c2.metric("Ingevuld",st.session_state.answered); c3.metric("Resterend",100-st.session_state.answered); c4.metric("Tijd",format_time(st.session_state.test_deadline-time.time()))
            ...
            st.progress(st.session_state.answered/st.session_state.rounds,text=f"Vraag {st.session_state.answered+1} van {st.session_state.rounds}")
        ...
        if st.session_state.answer_mode=="Meerkeuze":
            ...
            for i,ch in enumerate(st.session_state.choices):
                with cols[i%2]: st.button(str(ch),key=f"c{st.session_state.answered}_{ch}",use_container_width=True,on_click=process_answer,args=(ch,),disabled=st.session_state.locked)
        else:
