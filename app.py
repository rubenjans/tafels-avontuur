...
.fireworks{position:fixed;inset:0;pointer-events:none;z-index:9999;overflow:hidden}.firework{position:absolute;width:8px;height:8px;border-radius:50%;animation:burst 1.8s ease-out infinite;box-shadow:0 -70px #ff4d6d,49px -49px #ffd166,70px 0 #06d6a0,49px 49px #4cc9f0,0 70px #9b5de5,-49px 49px #ff9f1c,-70px 0 #f72585,-49px -49px #00f5d4}.f1{left:20%;top:35%;animation-delay:0s}.f2{left:50%;top:22%;animation-delay:.55s}.f3{left:78%;top:38%;animation-delay:1.05s}.f4{left:35%;top:60%;animation-delay:1.35s}@keyframes burst{0%{transform:scale(.05);opacity:1}65%{transform:scale(1.35);opacity:1}100%{transform:scale(1.8);opacity:0}}
</style>
...
def process_answer(value):
    ...
    if value==st.session_state.a*st.session_state.b:
        ...
        if not test:
            ...
            st.session_state.feedback=f"✅ Juist! +{10+bonus} punten en +{20+bonus} XP"; update_badges()
    else:
...
if not st.session_state.started:
    ...
    def game_panel():
        if not st.session_state.finished:
            ...
            if st.session_state.level=="Met tijdsklok" and now>=st.session_state.deadline: skip_timed_question(); st.rerun()
        if st.session_state.finished:
            fireworks()
            total=st.session_state.answered; pct=round(100*st.session_state.correct/max(1,total),1)
            elapsed=st.session_state.finished_at-st.session_state.game_start
            medal="🏆" if pct>=90 else "🌟" if pct>=70 else "💪"
            st.markdown(f'<div class="card" style="text-align:center"><div style="font-size:4rem">{medal}</div><h2>Resultaat</h2><p>{st.session_state.end_reason}</p><h1>{pct}%</h1><p>Benodigde tijd: <b>{format_time(elapsed)}</b></p></div>',unsafe_allow_html=True)
            c1,c2,c3,c4=st.columns(4); c1.metric("✅ Juist",st.session_state.correct); c2.metric("❌ Fout",st.session_state.wrong); c3.metric("⏭️ Geen antwoord",st.session_state.timed_out); c4.metric("⏱️ Tijd",format_time(elapsed))
        ...
        if st.session_state.level=="Toetsniveau":
            c1.metric("Vraag",f"{st.session_state.answered+1}/100"); c2.metric("Ingevuld",st.session_state.answered); c3.metric("Resterend",100-st.session_state.answered); c4.metric("Tijd",format_time(st.session_state.test_deadline-time.time())); st.progress(st.session_state.answered/100)
        else:
            c1.metric("⭐ Score",st.session_state.score); c2.metric("🔥 Reeks",st.session_state.streak); c3.metric("❤️ Levens",st.session_state.lives); c4.metric("⚡ XP",st.session_state.xp); st.progress(st.session_state.answered/st.session_state.rounds,text=f"Vraag {st.session_state.answered+1} van {st.session_state.rounds}")
